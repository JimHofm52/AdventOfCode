...
public class Day06 {
     ...
     * 
     * @param inStr, array of direct orbits. IE, COM orbited by B (direct orbit
     *               count = 1)
     *               or B orbited by C (dirct count = 1 plus C indirectly orbits
     *               COM, indirect count = 1)
     *               therefore it's orbit count = 2.
     */
    private static int findOrb(Orbit[] inStr, String srch) {
        for (int i = 0; i < inStr.length; i++) {
            if (inStr[i].ctr.equals(srch))
                return i;
        }
        return -1;
    }
